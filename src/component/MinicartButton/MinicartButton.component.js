...
import './MinicartButton.style.scss';

export class MinicartButton extends PureComponent {
  static propTypes = {
  ...
  renderSVG() {
    ...
    const SVGClassNames = classnames('MinicartSVG', { 'MinicartSVG-Large': !isButton });
    return (
      <svg
  ...
  renderWrapper() {
    const { onMinicartButtonClick, isButton } = this.props;

    return isButton ? (
      <button
        tabIndex={0}
        type="button"
        className="MinicartButtonWrapper"
      ...
      <div className="MinicartSVG-Color">{this.renderSVG()}</div>
    );
  }
